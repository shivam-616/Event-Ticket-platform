...
import com.event.tickets.exeception.EventUpdateException;
...
public class EventServiceIml implements EventService {
    ...
    public Event updateEventForOrganizer(UUID organizerId, UUID id, UpdateEventRequest event) {
        ...
        if (!id.equals(event.getId())) {
            throw new EventUpdateException("Event ID in path and request body do not match");
        }
        Event existingEvent = eventRepository.findByIdAndOrganizerId(id, organizerId)
                .orElseThrow(() -> new EventUpdateException(
                        String.format("Event with ID '%s' for organizer ID '%s' not found", id, organizerId))
                );
